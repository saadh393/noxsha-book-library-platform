...
export async function adminLogin(email: string, password: string) {
  ...
  return handleResponse<{ user: { id: string; email: string; name: string } }>(response);
}
...
export async function getAdminSession() {
  ...
  return handleResponse<{ user: { id: string; email: string; name: string } | null }>(response);
}
...
export async function updateBook(id: string, payload: Partial<Omit<Book, 'id' | 'created_at'>>) {
...
export async function fetchReviews(filter: 'all' | 'approved' | 'pending') {
  ...
  return handleResponse<{ data: Array<Review & { book_title: string | null }> }>(response);
}
...
export async function updateSocialLink(id: string, payload: Partial<SocialLink>) {
...
export async function createNavLink(payload: { label: string; href: string; display_order: number }) {
  const response = await fetch('/api/navigation', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
...
export async function updateHighlightCategory(id: string, payload: Partial<HighlightCategory>) {
...
export async function updateHighlightService(id: string, payload: Partial<HighlightService>) {
